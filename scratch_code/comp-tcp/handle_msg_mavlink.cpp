...
void get_messages(void)
{
    ...
    // Read bytes from the serial port and store them in the buffer
    for (ssize_t i = 0; i < n; ++i)
    {
        buffer[i] = read_serial_port();
    }
    ...
    for (unsigned int i = 0; i < n; ++i)
    {
        ...
        {
            fprintf(stderr,"There was an error parsing the message, but we can keep trying to receive bytes\n");
            break;
        }
    }
}